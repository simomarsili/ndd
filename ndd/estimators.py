...
def check_input(fit_function):  # pylint: disable=no-self-argument
    ...
    @wraps(fit_function)
    def wrapper(obj, nk, zk=None, k=None):
        nk = obj.check_nk(nk)
        if zk is not None:
            zk = obj.check_nk(zk)
        k = obj.check_k(k)
        return fit_function(obj, nk, zk=zk, k=k)
...
class EntropyEstimator(BaseEstimator, ABC):
    ...
    def __call__(self, nk, zk=None, k=None):
        """Fit and return the estimated value."""
        return self.fit(nk, zk=None, k=k).estimate_
...
class Grassberger(EntropyEstimator):
    ...
    @check_input
    def fit(self, nk, zk=None, k=None):  # pylint: disable=unused-argument
        """
        Parameters
        ----------
        nk : array-like
